...
function new_post_animation(post) {
    ...
    if (post["first_name"] == '' && post["last_name"] == '') {
        username_link.innerHTML = post["user"].charAt(0).toUpperCase() + post["user"].slice(1);
    } else {
        username_link.innerHTML = post["first_name"].charAt(0).toUpperCase() + post["first_name"].slice(1) + ' ' + post["last_name"].charAt(0).toUpperCase() + post["last_name"].slice(1)
    }
    bold.append(username_link);1
...
function follow() {
    // get all required fields
    let following = document.querySelector('#profile_username').innerHTML.toLowerCase();
...
function follow_status() {
    ...
    .then(result => {
        document.querySelector('#follow').innerHTML = (result["is_following"] ? 'Unfollow' : 'Follow');
        if (result["first_name"] != '' || result["last_name"] != '') {
            document.querySelector('#follow-message').innerHTML = (result["is_following"] ? 
            `You are following ${result["first_name"].charAt(0).toUpperCase() + result["first_name"].slice(1)} 
            ${result["last_name"].charAt(0).toUpperCase() + result["last_name"].slice(1)}` 
            : `You are not following ${result["first_name"].charAt(0).toUpperCase() + result["first_name"].slice(1)} 
            ${result["last_name"].charAt(0).toUpperCase() + result["last_name"].slice(1)}`);
        } else {
            document.querySelector('#follow-message').innerHTML = (result["is_following"] ? 
            `You are following ${result["username"].charAt(0).toUpperCase() + result["username"].slice(1)}` 
            : `You are not following ${result["username"].charAt(0).toUpperCase() + result["username"].slice(1)}`);
        }
        document.querySelector('#profile_followers').innerHTML = `${result["followers"]} followers`;
        document.querySelector('#profile_following').innerHTML = `${result["following"]} following`;
    })
}
...
function like_count(post_id) {
    ...
    .then(result => {
        ...
        document.getElementById(`${id[0]}_like_count`).innerHTML = `Liked by: <strong>${result["likes"]}</strong>`;
    })
}
...
function edit_post(edit) {
    ...
    let current_content = document.getElementById(`${post_id.split("_")[0]}_content`);
...
function change_post_view(post_id) {
    ...
    .then(response => response.json())
    .then(result => { // {post.id_image}
        document.getElementById(`${post_id.split("_")[0]}_content`).innerHTML = result["content"];
    })
}
...
function get_followers() {
    username = document.getElementById('profile_username').innerHTML.toLowerCase();
...
function generate_follow_list(users, string) {
    if (string == 'follower') {
        profile_username = document.getElementById('profile_display_name').innerHTML;
        document.getElementById('users_modal_message').textContent = `${profile_username}'s followers`;
    } else if (string == 'following') {
        profile_username = document.getElementById('profile_display_name').innerHTML;
        document.getElementById('users_modal_message').textContent = `${profile_username} following`;
    } else if (string == 'users_like') {
        document.getElementById('users_modal_message').textContent = `People who like this post`;
    }
    ...
    if (users.length > 0) {
        for (let i = 0; i < users.length; i++) {
            ...
            if (users[i].first_name == '' && users[i].last_name == '') {
                username_link.innerHTML = users[i].username.charAt(0).toUpperCase() + users[i].username.slice(1);
            } else {
                username_link.innerHTML = users[i].first_name.charAt(0).toUpperCase() + users[i].first_name.slice(1) + ' ' + users[i].last_name.charAt(0).toUpperCase() + users[i].last_name.slice(1);
            }
...
function get_next_post(pathname) {
    ...
    .then(result => {
        ...
        if (JSON.parse(result).body.length > 0) {    
            infinite_scroll(JSON.parse(result).body, JSON.parse(result).total_post_count, pathname);
        }
        ...
        if (JSON.parse(result).total_post_count <= 5) {
            ...
            if (typeof(login) != 'undefined' && login != null) {
                ...
                if (login.innerHTML.toLowerCase() == pathname) {
                    document.querySelector('.please_wait').innerHTML = 'Your followers are waiting for more posts';
                }
                else {  
                    document.querySelector('.please_wait').innerHTML = `Stay tuned for more posts`;
                }
            }
            else {  
                document.querySelector('.please_wait').innerHTML = `Stay tuned for more posts`;
...
function infinite_scroll(post, total_post_count, pathname) {
    ...
    for (let i = 0; i < post.length; i++) {
        ...
        if (post[i]["first_name"] == '' && post[i]["last_name"] == '') {
            username_link.innerHTML = post[i]["user"].charAt(0).toUpperCase() + post[i]["user"].slice(1);
        } else {
            username_link.innerHTML = post[i]["first_name"].charAt(0).toUpperCase() + post[i]["first_name"].slice(1) + ' ' + post[i]["last_name"].charAt(0).toUpperCase() + post[i]["last_name"].slice(1)
        }
        bold.append(username_link);1
        username.append(bold);
        ...
        if (document.getElementById('all_posts').childElementCount == total_post_count) {
            // mark end of page
            end_of_page = true;
            ...
            if (pathname == 'home') {
                ...
                document.querySelector('.please_wait').style.display = 'block';
                document.querySelector('.please_wait').innerHTML = 'Stay tuned or follow more people to see more posts';
            } else {
                document.querySelector('.please_wait').style.display = 'block';
                login = document.getElementById('username');
                if (typeof(login) != 'undefined' && login != null) {
